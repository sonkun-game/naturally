<template>
  <div class="relative w-screen h-screen overflow-hidden bg-black font-sans select-none">
    ...
    <Transition enter-active-class="transition duration-500 ease-out" enter-from-class="opacity-0"
      ...
      <div v-if="isLoading || isTransitioning"
        ...
        <div class="w-64 h-1.5 bg-gray-800 rounded-full overflow-hidden border border-cyan-500/30">
          <div class="h-full bg-gradient-to-r from-cyan-500 via-fuchsia-500 to-emerald-400 transition-all duration-300"
            :style="{ width: (isTransitioning ? 100 : loadProgress) + '%' }"></div>
        </div>
      </div>
    </Transition>
    ...
    <div v-if="showHotspots && !isLoading" class="absolute inset-0 z-20 pointer-events-none overflow-hidden">
      <div v-for="spot in currentHotspots" :key="spot.id" v-show="spot.visible"
        ...
        <div class="relative flex items-center justify-center">
          ...
          <template v-if="spot.type === 'teleport'">
            <div class="w-14 h-14 rounded-full bg-fuchsia-500/30 border-2 border-fuchsia-400 animate-ping absolute">
            </div>
            <div
            ...
            <!-- Portal Label Badge -->
            <div
              class="absolute left-12 top-1/2 -translate-y-1/2 bg-gray-950/95 backdrop-blur-md border border-fuchsia-400 text-fuchsia-200 text-xs px-3.5 py-2 rounded-xl whitespace-nowrap shadow-2xl group-hover:translate-x-1 transition-all flex items-center gap-2 font-mono font-semibold">
              <Sparkles class="w-3.5 h-3.5 text-fuchsia-400 animate-spin" />
              <span>{{ spot.title }}</span>
              <span
                class="bg-fuchsia-500/30 px-1.5 py-0.5 rounded text-[10px] uppercase text-fuchsia-300 border border-fuchsia-400/40">CLICK
                TO ENTER</span>
            </div>
          </template>
          ...
          <template v-else>
            ...
            <div
              class="w-8 h-8 rounded-full bg-cyan-900/90 border border-cyan-300 shadow-lg flex items-center justify-center text-cyan-300 group-hover:scale-125 transition-transform duration-300">
              <Info class="w-4 h-4" />
            </div>
            <div
              class="absolute left-10 top-1/2 -translate-y-1/2 bg-gray-950/90 backdrop-blur-md border border-cyan-500/40 text-cyan-300 text-xs px-3 py-1.5 rounded-xl whitespace-nowrap shadow-xl opacity-80 group-hover:opacity-100 group-hover:translate-x-1 transition-all">
              {{ spot.title }}
            </div>
          </template>
        </div>
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95"
      ...
      <div v-if="activeHotspot"
        ...
        <div
          class="bg-gray-900/95 border border-cyan-500/40 text-white rounded-3xl max-w-lg w-full p-6 shadow-2xl relative">
          ...
          <div class="flex items-center gap-3 mb-4">
            <div
              class="w-10 h-10 rounded-2xl bg-cyan-500/20 border border-cyan-400 flex items-center justify-center text-cyan-300">
    ...
    <footer class="absolute bottom-6 inset-x-0 z-30 flex justify-center pointer-events-none px-6">
      <div
        class="bg-gray-900/85 backdrop-blur-md border border-cyan-500/30 px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 md:gap-5 pointer-events-auto">
        <!-- Auto Rotate Toggle -->
        <button @click="toggleAutoRotate" :class="[
        ...
        <!-- Toggle Hotspots -->
        <button @click="showHotspots = !showHotspots" :class="[
